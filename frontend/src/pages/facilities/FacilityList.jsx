...
export default function FacilityList() {
  ...
  const generatePDF = (booking) => {
    ...
    // Header
    doc.setFontSize(20);
    doc.text('SLIIT Arena 360', 105, 20, { align: 'center' });
    ...
    // Details
    doc.setFontSize(12);
    const userName = booking.user?.name || user?.name || 'N/A';
    const facilityName = booking.facility?.name || 'N/A';
    
    doc.text(`User's Name: ${userName}`, 20, 50);
    doc.text(`Facility Name: ${facilityName}`, 20, 60);
    doc.text(`Date: ${booking.date}`, 20, 70);
    doc.text(`Time: ${booking.startTime} - ${booking.endTime}`, 20, 80);
    doc.text(`Purpose: ${booking.purpose}`, 20, 90);
    doc.text(`Status: ${booking.status}`, 20, 100);
    
    const fileName = `Receipt_${facilityName.replace(/\\s+/g, '_')}_${booking.date}.pdf`;
  ...
  return (
    <div>
            ...
            <tbody>
              {myBookings.map(b => (
                <tr key={b._id} className="hover:bg-gray-50">
                  ...
                  <td className="table-cell">
                    <button onClick={() => generatePDF(b)} className="btn-primary text-xs py-1 px-3">Download PDF</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {/* PDF Preview Modal */}
      <Modal isOpen={showPdfModal} onClose={() => setShowPdfModal(false)} title="Receipt Preview" size="lg">
        <div className="flex flex-col h-[60vh]">
          <iframe src={pdfDataUri} className="flex-1 w-full border border-gray-200 rounded mb-4" title="PDF Preview"></iframe>
          <div className="flex gap-3 mt-auto">
            ...
            <button onClick={() => setShowPdfModal(false)} className="btn-secondary px-5">
              Close
            </button>
          </div>
        </div>
